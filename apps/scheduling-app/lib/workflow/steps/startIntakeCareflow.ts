...
export async function startIntakeCareflowStep(
  ...
  if (!input.salesforceLeadId) {
    throw new Error('[startIntakeCareflowStep] Salesforce lead ID is required but was not provided');
  }
  ...
  if (!careflowId || !patientId) {
    ...
    throw new Error('Failed to start intake careflow - missing careflowId or patientId');
  }
